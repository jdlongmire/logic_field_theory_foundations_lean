...
noncomputable def collapsed_state
  ...
  ⟨[(measured, if a = 0 then 0 else a / Complex.abs a)],
   by simp,
   by simp [listNormSq]; split_ifs <;> simp⟩
...
theorem born_rule_from_strain (ψ : QuantumState) (G : Omega) :
  ...
  simp [collapsed_state, ampOf]
  split_ifs with h
  · -- Case: amplitude is 0
    simp [h]
  · -- Case: amplitude is non-zero, gets normalized to unit magnitude
